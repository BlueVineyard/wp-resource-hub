...
?>
        <div class="wprh-collection-resources-wrapper">
            <div class="wprh-collection-search">
                <input type="text" id="wprh-resource-search" class="widefat"
                ...
                <?php else : ?>
                    <?php foreach ($resource_ids as $resource_id) : ?>
                        ...
                        <div class="wprh-collection-resource-item" data-resource-id="<?php echo esc_attr($resource_id); ?>">
                            ...
                            <?php endif; ?>
                            <button type="button" class="wprh-remove-resource button-link"
    ...
    ?>
        <div class="wprh-collection-settings">
            <p>
                ...
                <select name="wprh_collection_display_style" id="wprh_collection_display_style" class="widefat">
                    <option value="list" <?php selected($display_style, 'list'); ?>>
                        <?php esc_html_e('List', 'wp-resource-hub'); ?></option>
                    <option value="grid" <?php selected($display_style, 'grid'); ?>>
                        <?php esc_html_e('Grid', 'wp-resource-hub'); ?></option>
                    <option value="playlist" <?php selected($display_style, 'playlist'); ?>>
                        <?php esc_html_e('Playlist (sidebar)', 'wp-resource-hub'); ?></option>
                    <option value="nested-accordion" <?php selected($display_style, 'nested-accordion'); ?>>
                        <?php esc_html_e('Nested Accordion', 'wp-resource-hub'); ?></option>
                </select>
            </p>

            <p>
                <label>
                    <input type="checkbox" name="wprh_collection_show_progress" value="1"
            ...
            <p>
                <label>
                    <input type="checkbox" name="wprh_collection_auto_advance" value="1"
...
<?php
    ...
    public function save_meta($post_id, $post, $update)
    {
        // Verify nonce.
        if (
            ! isset($_POST['wprh_collection_meta_nonce']) ||
        ...
        if (isset($_POST['wprh_collection_display_style'])) {
            $valid_styles = array('list', 'grid', 'playlist', 'nested-accordion');
